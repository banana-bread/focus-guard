...
// Serialises allow-rule creation so that concurrent calls cannot compute the
// same nextId and produce a duplicate-ID error.
let _allowRuleQueue = Promise.resolve();

/**
 * Adds a high-priority allow rule for a domain (unlocks it).
 * Calls are serialised via a module-level promise chain so concurrent
 * invocations never race on the nextId computation.
 */
export function unlockDomain(domain) {
  const result = _allowRuleQueue.then(async () => {
    const existing = await chrome.declarativeNetRequest.getDynamicRules();
    ...
    if (nextId > 19999) {
      throw new Error('Allow rule ID space exhausted (max 9999 concurrent unlocks reached)');
    }
  ...
  // Advance the queue with a non-rejecting version to keep the chain alive
  // even if this call fails; the original `result` still rejects for the caller.
  _allowRuleQueue = result.catch(() => {});
