...
export async function updateSettings(partial) {
  const { unlockDurationMinutes } = partial;
  if (
    ...
    throw new Error("unlockDurationMinutes must be an integer between 1 and 1440.");
  }
  const current = await getSettings();
  await set("settings", { ...current, unlockDurationMinutes });
}
