...
export async function importPublicKey(spkiBytes) {
  // Accept either ArrayBuffer or ArrayBufferView (TypedArray/DataView).
  // Always normalise to Uint8Array so the same object works across different
  // execution contexts (e.g. jsdom vm realms) where ArrayBuffer.slice() can
  // produce a cross-realm buffer that some native crypto backends reject.
  const keyData = spkiBytes instanceof ArrayBuffer
