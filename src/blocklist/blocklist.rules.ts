 ...
 * @returns A rule ID in the range [ALLOW_RULE_ID_BASE, ALLOW_RULE_ID_BASE + 499].
 */
function domainAllowRuleId(domain: string): number {
  ...
  return (hash % 500) + ALLOW_RULE_ID_BASE;
}
...
export async function syncRules(_oldList: string[], newList: string[]): Promise<void> {
  ...
  const removeRuleIds = existingRules
    .filter((r: chrome.declarativeNetRequest.Rule) => r.id >= BLOCK_RULE_ID_BASE && r.id < ALLOW_RULE_ID_BASE)
